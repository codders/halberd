...
def load(filename):
    ...
    for tup in reader:
        ...
        if not (isinstance(clue.headers, list) or
                isinstance(clue.headers, tuple)):
...
def _mkdir(dest):
    ...
    try:
        ...
        if not shutil.stat.S_ISDIR(st.st_mode):
            raise InvalidFile('%s already exist and is not a directory' % dest)
...
class ClueDir:
    ...
    def save(self, url, addr, clues):
        ...
        urldir = _mkdir(os.path.join(self.root, _sanitize(url)))
        filename = _sanitize(addr) + os.extsep + self.ext
